...
Orchestrates:
  1) BAU baseline (if needed)
  2) Negishi weights (from BAU or file) – saved to results_dir
  3) Cooperative planners (CRRA / FS), GRAND-only (cache-first)
  4) Noncooperative Nash (CRRA / FS), caches SINGLETONS
  5) Coalition game: ensure/cache requested coalitions for export

Design choices
--------------
- The cache is the single source of truth for coalition exports. We always
  initialize a CoalitionStore.
- All cached solutions must contain discount factors `disc[(r,t)]`. We store
  *discounted* lifetime payoffs only (computed at write time).
...
def run_analysis(config: dict, *, diagnostics_dir: Path) -> AnalysisResult:
    ...
    try:
        T = int(config["T"])
        # Decadal by construction; allow configs without tstep and default to 10
    ...
    logger.info(
        "RUN | T=%s, tstep=%s | planners(crra=%s, fs=%s) | nash(crra=%s, fs=%s) | "
    ...
    if need_bau:
        logger.info("Running BAU (utility=CRRA; μ policy Excel-style; S fixed to BAU).")
