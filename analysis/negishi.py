...
This module provides:
  - CSV I/O helpers for reading/writing period-by-period Negishi weights
  - Construction of Negishi weights from a BAU solution under:
      * CRRA marginal utility proxy
      * Fehr–Schmidt (FS) marginal utility proxy
  - An optional variant that scales FS marginal utilities by an externally
    provided FS discount series (used in the "fs_after_disc" pipeline).
...
def load_negishi_weights_from_csv(
    ...
    *,
    ...
    Accepted input shapes:
      ...
      - Period labels may be ints or strings (e.g., "1", "Y1", "y12"); coerced to ints.
        Note: only periods in 1..T are kept; anything outside is dropped.
...
def compute_negishi_weights_from_bau(
    ...
    *,
    ...
    Notes
    -----
    - `bau_sol["C"]` is expected to be a dict keyed by (region, period) tuples.
    - Regions and periods are inferred from `params.L.index` and `bau_sol["C"]`.
...
def compute_negishi_weights_from_bau_fs_after_disc(
    ...
    *,
    ...
    """
    Compute FS-based Negishi weights where the FS marginal-utility proxy is *scaled*
    by an externally provided discount series (the "fs_after_disc" pipeline).

    Concretely:
        MU_fs_after_disc(r,t) = MU_fs(r,t) * fs_disc(r,t)
        w(r,t) ∝ 1 / MU_fs_after_disc(r,t), normalized so sum_r w(r,t) = 1.

    Parameters
    ----------
    bau_sol:
        BAU solution (expects `bau_sol["C"]` keyed by (region, period)).
    fs_disc:
        Dict keyed by (region, period) giving the discount factor used for scaling.
        (If a key is missing, current code will raise at runtime; consider defaulting to 1.0.)
    population_weight_envy_guilt:
        Whether FS envy/guilt shares use population-weighting (must match model setting).
    output_path:
        Optional path to write the resulting weights CSV.
    """
    regions = list(params.L.index)
    periods = sorted({t for (_, t) in bau_sol["C"]})
    ...
    for (r, t), C_rt in bau_sol["C"].items():
        ...
        MU[(r, t)] = fs_negishi_mu(
            ...
            C=bau_sol["C"], L=params.L,
            population_weight_envy_guilt=population_weight_envy_guilt,
        ) * fs_disc.get((r, t))  # Scale MU by fs_disc (fs_after_disc pipeline)
