...
---------------------
- Loads the *exogenous inputs* and *warm-start initializations* used by the solver.
- Validates the horizon: T must be <= 59 (data coverage).
- Assumes a fixed 10-year model grid (periods 1..T). No subdecadal support.

Input conventions
-----------------
- CSV filenames follow the original RICE/RICE13 naming.
- Region codes are read from Region_names.csv and used to align all other tables.
- FS_types.csv is expected to have countries on rows (if not, we transpose).

Warm starts
-----------
- Country×period warm starts are loaded from the dedicated *_init.csv files.
- Global warm-start series for climate/SLR (e.g., M_at, T_at, gsic/gis/ais, slr_TE, slr)
  are loaded from Var_country_independent_init.csv.

Notes on "alpha"/"eta"
----------------------
- inv_p['eta'] is a scalar parameter loaded from Time_countries_invariant_param.csv.
- Countries_variant_param.csv['alpha'] is also read and exposed as `crra_eta` (per-region curvature)
  while keeping `alpha` in Params for compatibility with the original dataset layout.
"""
...
def interpolate_series(df: pd.DataFrame, T: int) -> pd.DataFrame:
    """
    Fixed-grid (10-year period) pass-through with optional warm-start.
    ...
    Input df may have a warm-start column '0'. We:
      ...
      4) Return columns 1..T (ints, interpreted as model periods), same index, float dtype.
    """
...
# ---------------------------
# Loader (fixed 10-year grid inputs)
# ---------------------------

def load_params(data_path: Path, T: int) -> Params:
    """Load all parameters/series for T model periods (fixed 10-year grid). Validates T <= 59."""
    ...
    # Emissions intensity (sigma; 10-year dynamics)
    ...
    sigma = sig.copy()
    logger.info("Sigma: built from Sig_0 and g_sig dynamics (10-year periods)")

    # Land-use emissions (E_land; 10-year periods)
    ...
    for r in countries:
        ...
        for j in range(1, T + 1):
            if j == 1:
                ...
                eland.at[r, j] = eland.at[r, j-1] * (1 - d_el[r])
    logger.info("E_land: built from Eland_0 and d_el (10-year periods)")

    # Exogenous forcing f_ex (0..T), standard 10-year path
    ...
    # Backstop / θ1 (backstop price path + θ1 = p_backstop * sigma / th_2)
    #
    # base_year and backstop_switch_year are dataset/model constants used throughout the repo.
    # backstop_switch_year=2250 matches the μ step policy used in BAU seeding elsewhere.
    ...
    for c in countries:
        ...
        for j in range(max(T+1, 60)):
            ...
            if j == 0:
                ...
                if year <= backstop_switch_year: # ≤2245 on 10-year grid (2255 is the first >2250)
    ...
    # Global climate/SLR warm starts come from Var_country_independent_init.csv
