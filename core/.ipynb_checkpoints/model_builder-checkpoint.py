"""
Constructs the Pyomo ConcreteModel for the RICE13_FS IAM,
using preprocessed data in the Params object.

Key design choices (2025):
- Time grid is **decadal only** (fixed 10-year step inside the model).
- Sea level rise (SLR) is always endogenous. We warm-start from params,
  but we do not allow an exogenous SLR path at runtime.
- CRRA utility is population-weighted by construction.
- FS utility's objective is always population-weighted (multiply by L).
  You can still toggle whether envy/guilt are population-weighted across
  countries via `population_weight_envy_guilt`.
- AIS melt around 3°C is smoothed via a logistic blend (IPOPT-friendly).
"""
...
def build_model(
    ...
    *,
    ...
    if exogenous_S is not None:
        ...
        for r in m.REGIONS:
            for t in m.T:
                # Clamp defensively into [0,1] before fixing to avoid bound complaints
                v = float(exoS.at[r, t])
                if v < 0.0:
                    v = 0.0
                if v > 1.0:
    ...
    # Per-(r,t) discount factor Param so all solvers can use m.disc in their objectives.
    ...
    if utility == 'crra':
        ...
        # --- Smooth positive part: stable surrogate for max(x, 0) ---
        # Units: FS block uses per-capita consumption in "thousands",
        # so eps is also in those units. eps=1e-4 is a good default for IPOPT.
