...
Core changes (2025 refactor)
----------------------------
- Decadal-only core model: we no longer pass `tstep` into `model_builder`.
  (We still accept `tstep` in this function to seed μ using calendar years.)
...
- Convergence guard: require at least one successful best-response update per
  outer iteration to avoid false convergence (tracked via `made_update`).
...
def _init_profiles_from_solution(
    ...
    μ by year threshold: μ=1 if year >= backstop_switch_year else 0.
    """
...
def solve_nash(
    ...
    *,
    ...
    # --- Discounting (FS only) ---
    discount_series: Optional[Dict[int, float]] = None,  # per-period global series d_t (t=1..T)
    ...
    for k in range(1, max_iter + 1):
        ...
        # Track μ-residual to best response vs iteration baseline
        ...
        for r_star in region_order:
            ...
            # Decadal-only core: do NOT pass tstep into builder
        ...
        # Check relaxed-change Δ on both μ and S (diagnostic; optionally ignore last period)
        ...
        # that the max residual be below tol, plus at least one successful update.
        conv_resid = bool(resid_updated and (max_resid_iter <= tol))
        conv = bool(conv_resid and made_update)

        diag_evo.append({
            "iteration": k,
            # 'max_delta' now records the μ-residual to best response.
    ...
    # Final evaluation with fixed profiles to collect a consistent portfolio
    ...
    # REQUIRED for discounted payoffs downstream
