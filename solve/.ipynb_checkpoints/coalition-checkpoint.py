...
Notes:
  ...
  • Robust to IPOPT failures: skip profile updates for the failing block and continue.
  • Periods are model periods 1..T (years are derived elsewhere).
...
def _evaluate_fixed(
    ...
    """
    # Decadal-only core: do NOT pass tstep into the builder.
    # If exogenous_S_df is provided, the builder is the SINGLE place that fixes S.
    m = model_builder.build_model(
        ...
        # NEW: allow injected discount series (global or regional)
...
def solve_coalition_game(
    ...
    *,
    ...
    # NEW: discounting controls for FS (optional)
    ...
    # STRICT: If FS is run with an explicit discount series (file/one_pass/two_pass),
    # a disc_tag must be provided by the orchestrator (no silent fallbacks).
    ...
    for k in range(1, max_iter + 1):
        ...
        mA = model_builder.build_model(
            ...
            exogenous_S=exoS,
            population_weight_envy_guilt=population_weight_envy_guilt,
            # NEW: thread through coalition-level discount series
            discount_series=discount_series,
        )
        ...
        # ---- Convergence metrics: residual (primary) and relaxed Δ (diagnostic)
        ...
        # OLD
        #diag_evo.append({"iteration": k, "max_delta": float(max_d)})

        # NEW
        # --- Per-region μ-deltas for diagnostics (per period, per iteration)
        #
        # We only track μ here because in the exogenous-S runs of interest S is
        # fixed by construction (so ΔS ≈ 0). The structure is generic enough to
        # be extended later if needed.
        #
        # Respect the same ignore_last_k_periods logic as the global max Δ.
        ...
        diag_evo.append({
            "iteration": k,
            # 'max_delta' now records the μ-residual to best response (R^{(k)}).
            "max_delta": float(max_resid_iter if resid_updated else float("inf")),
            # Keep relaxed-change max Δ as a diagnostic field.
            "max_delta_relaxed": float(max_d),
            "per_region": per_region,
        })

# noisy residuals for debugging
#        if logger.isEnabledFor(logging.INFO):
#            for r, payload in per_region.items():
#                mu_series_str = " ".join(f"{d:.3e} \n" for d in payload["mu_deltas"])
#                logger.info(
#                    "Iter %d region %s Δμ (ignoring last %d periods):\n %s",
#                    k, r, ignore_last_k_periods, mu_series_str,
#                )
        ###########################################
        # END NEW
    ...
    # Final evaluation with locked profiles to collect a consistent full solution (with disc)
    try:
        plot_nonconv_diag(
            diag_evo,
    ...
    try:
        sol = _evaluate_fixed(
            ...
            exogenous_S_df=exoS,
            # NEW: final evaluation must match the same discounting
            discount_series=discount_series,
         )
    except Exception as e:
    ...
    # Build spec_id WITHOUT tstep (decadal-only pipeline; matches planner/BAU)
    spec_id = build_solution_spec_id(
        ...
        # NEW: make coalition cache keys discount-aware for FS runs
