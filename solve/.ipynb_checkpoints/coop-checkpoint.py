...
Notes:
- The `tstep` argument is retained for signature compatibility with callers, but the
  model itself assumes a 10-year step internally and does not use `tstep`.
"""
...
def solve_planner(
    params,
    T: int,
    tstep: int,  # kept for call-site compatibility; unused by the core model (decadal-only)
    *,
    utility: str,
    ...
    negishi_use: bool,
    # --- Discounting (FS only) ---
    discount_series: Optional[Dict[int, float]] = None,  # per-period series (global or regional applied upstream)
    ...
    tstep : int
        Years per period (retained for compatibility; the model is decadal-only).
    utility : {'crra','fs'}
    ...
    discount_series : dict[int,float] or None
        Optional global discount factor series {t -> d_t} to override the geometric path from
        per-region rho. When provided, it is applied to all regions uniformly (FS-only usage).
    ...
    # This is the SCC in money units under a world-wide, equal-per-capita
    # lump-sum numéraire. It inherits the Negishi / FS / CRRA weighting from
