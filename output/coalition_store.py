...
- Solution blobs are optional: if a blob cannot be serialized, we still upsert the registry row
  (with NULL blob) and mirror lightweight identity fields into meta_json for filtering/export.
...
class CacheEntry:
    ...
    meta: Dict[str, Any]   # e.g., {"converged": True, "iterations": 37, "S_tag": "...", "utility": "...", "disc_tag": "..."}
...
class CoalitionStore:
    ...
    def put(
        ...
        *,
        ...
        # Serialize solution to BLOB (best-effort).
        # If serialization fails, we still write the registry row with NULL blob so:
        #   - cache hits still exist at the registry level
        #   - exporters can still filter/select entries via mirrored meta_json
    ...
    def _open_db(db_path: Path) -> sqlite3.Connection:
        ...
        # timeout allows waiting for short concurrent locks; isolation_level=None enables autocommit.
        conn = sqlite3.connect(str(db_path), timeout=30.0, isolation_level=None, detect_types=0)
        cur = conn.cursor()
        # WAL journaling + NORMAL synchronous → robust + fast enough for frequent small upserts.
        with contextlib.suppress(Exception):
    ...
    def get_latest_fs(self, vec, disc_tag: str | None = None):
        """
        Convenience accessor: return the most recent FS entry for this coalition vector.

        - Uses meta_json fields mirrored at write time ("utility", "disc_tag") to filter.
        - Requires SQLite JSON1 for json_extract(...) (present in standard Python builds).
