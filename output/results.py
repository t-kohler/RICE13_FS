...
- Convert solver outputs into a consistent Excel workbook layout:
    * one sheet per region + a "global" sheet
    * a readable "config" sheet (display-only)
    * a "mu Plot" sheet (abatement rate over time)
    * coalition exports additionally include stability diagnostics sheets
- Enforce "single source of truth" helpers:
    * payoffs use payoff_row_discounted(...) consistently
    * coalition stability uses evaluate_stability_for_mask(...) consistently

Note: coalition exports are cache-first (streamed from CoalitionStore).
"""
...
# --- Helper: pretty, safe config for the Excel "config" sheet (display-only)
...
# ---- Debug helpers (exact utilities & deltas; logging-only, opt-in) ----
...
def export_tabular(
    ...
    *,
    ...
    # Timestamped filename; append "_disc" for FS runs when FS discounting is enabled (naming only).
    ...
    *,
    ...
    if utility.lower() == "fs":
        ...
        for r in cand:
            ...
            # Fallback to meta when blob is missing (NULL blob entries are allowed by design).
            ...
            # We require disc_tag to match the requested fs_disc_mode *exactly* for aligned runs:
            #   - one_pass exports only one_pass-tagged solutions
            #   - two_pass exports only two_pass-tagged solutions
            if want_aligned:
    ...
    for r in rows:
        ...
        # Diagnostics (log only): show what we are exporting / why a row was selected.
        logger.debug(
        ...
        # Diagnostics: compact audit table showing deviator PV utilities and deltas
...
def _attach_mu_plot(
    ...
    *,
    ...
    """
    Attach a "mu Plot" sheet (abatement rate μ over time).

    This helper supports a legacy caller-controlled switch: pass diagnostics_level="off"
    to skip the plot. Note that this is not automatically tied to config.yaml's
    diagnostics setting; coalition export currently calls this unconditionally.
    """
...
#    if backstop_switch_year is not None:
#        cap_year = min(base_year + max(periods)*tstep, int(backstop_switch_year) + tstep)
    # Presentation choice: cap the plot horizon to keep the Excel image readable.
    # If you want this tied to model horizon/backstop, re-enable the computed cap above.
    cap_year = 2125
    plot_mu_for_run(
    ...
    if cap_year is not None:
        ws["A65"] = f"x-axis limited (cap = {cap_year})"
