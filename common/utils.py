...
- Period ↔ year mapping (fixed 10-year grid in this repo; `tstep` args are legacy-only)
...
- Fingerprints / spec IDs (no `tstep`; optional Negishi digest; optional FS discount tag)
- Lightweight diagnostics plotting

Logging goes through `logging`; a few opt-in debugging helpers may print when called.
"""
...
# Fixed 10-year model grid (RICE-2013 style). Most of the code assumes this implicitly.
DECADAL_STEP = 10  # years per model period

# ------------------------------------------------------------------
# Global diagnostics switch (set once by cli.py after config load)
# Meaning here: controls *to-disk* diagnostics only (files), NOT logging verbosity.
...
def years_from_periods(periods, base_year, tstep):
    ...
    Note:
      `tstep` is kept for legacy call sites. In this repository the grid is fixed
      at 10-year periods (DECADAL_STEP), so callers should pass 10 (or omit this
      function entirely and use DECADAL_STEP directly).
    """
...
def _S_solution_to_df(S_map: dict, countries: List[str], T: int) -> pd.DataFrame:
    """Legacy helper: dict[(region,t)] -> DF. Prefer _solution_S_to_df() for new call sites."""
...
def build_ipopt(options: Optional[Dict[str, Any]] = None, log_path: Optional[Path] = None) -> pe.SolverFactory:
    ...
    for k, v in flat_opts.items():
        ipopt.options[k] = v
    # Only attach an IPOPT output file when diagnostics are ON (file output switch).
...
def print_most_violated_constraints(model, max_print: int = 20, tol: float = 1e-7) -> None:
    """Debug helper: print up to `max_print` constraints with violation > tol.

    This is intentionally a *printing* helper (opt-in). Core solver paths use logging.
    """
    viols = []
...
def build_config_fingerprint(config: Dict[str, Any], regions: Sequence[str]) -> Dict[str, Any]:
    ...
    Spec-relevant fingerprint (no `tstep`; time grid is fixed). Updated for split
    coalition config (CRRA/FS).
    """
...
def build_solution_spec_id(
    *,
    ...
    No `tstep` (time grid is fixed at 10-year periods). Includes optional Negishi
    digest if negishi_use=True.
...
def fetch_or_solve_coalitions(
    *,
    ...
    params,
    tstep: int,                                    # legacy-only; should be 10 in this repo (used for year mapping)
